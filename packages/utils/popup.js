...
let _destory = function(item) {
	if (item) {
		...
		let index = instances.findIndex(
			(itemEl) => itemEl.$options.name == item.$options.name,
		)
		if (index >= 0) {
...
class myDialog {
	...
	popup(options) {
		...
		return new Promise((resolve, reject) => {
			let constructor = this.$pcTpl.vue.extend(this.case)
			...
			for (let i = 0; i < instances.length; i++) {
				...
				if (instances[i].$options.name == this.case.name) {
					exit = true
					break
				}
			}
