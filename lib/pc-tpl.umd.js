...
var CNPrint_CNPrint = /*#__PURE__*/function () {
  ...
  _createClass(CNPrint, [{
    ...
    value: function initSocket() {
      ...
      try {
        __socket.onmessage = function (event) {
          ...
          var cb = JSON.parse(data.requestID.split('#')[1] || "{}"); // 回调数据
          ...
          switch (data.cmd) {
            case 'getPrinters':
              ...
              _this[cb.method].apply(_this, _toConsumableArray(cb.params));

              break;

            case 'print':
              if (data.status === 'success') {// 请求调取后台接口,打印回调
              } else {
    ...
    key: "sendCmd",
    value: function sendCmd(cmd, extraRequest) {
      try {
        var request = {
          version: '1.0',
        ...
        if (extraRequest) {
          request = CNPrint_objectSpread(CNPrint_objectSpread({}, request), extraRequest);
        }
